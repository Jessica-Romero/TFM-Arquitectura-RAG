...
def build_docs_images() -> None:
    ...
    Lee:
      - data/staged/rutas_nm.csv            (metadata de rutas)
      - data/staged/rutas_imagenes.csv   (rutas e imágenes asociadas)

    Escribe:
      - data/rag_docs/docs_images_ve.csv
    ...
    output_csv = rag_docs_dir / "docs_images_ve.csv"  # versión 09/12/2025
    ...
    for _, row in df_merged.iterrows():
        ...
        nombre_ruta = str(row.get("nombre_ruta_org", "") or "").strip()
        pueblo = str(row.get("pueblo_org", "") or "").strip()
        zona = str(row.get("zona_org", "") or "").strip()
        comarca = str(row.get("comarca_org", "") or "").strip()
        provincia = str(row.get("provincia_org", "") or "").strip()
        region = str(row.get("region_org", "") or "").strip()
        pais = str(row.get("pais_org", "") or "").strip()
