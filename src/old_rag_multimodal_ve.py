...
# ============================================================
# CREACIÓN DE FILTROS CON GEMINI
# ============================================================

gemini_filter_extractor = GoogleAIGeminiGenerator(model="gemini-2.0-flash")
...
Campos disponibles: {', '.join(FILTER_SCHEMA)}
...
Ejemplos:
- "Rutas en Barcelona" → [{{"field": "meta.provincia", "operator": "==", "value": "Barcelona"}}]
- "Sendero de menos de 10 km" → [{{"field": "meta.distancia_total_km", "operator": "<=", "value": 10}}]
- "Rutas cerca de El Bruc" → [{{"field": "meta.poblacion_cercana", "operator": "==", "value": "El Bruc"}}]
...
"""
    ...
    try:
        ...
        if isinstance(data, list):
            # Asegurar que todos los campos empiezan con meta.
            for item in data:
        ...
        print(f"Respuesta: {text[:200] if 'text' in locals() else 'N/A'}")
        return None
...
#
# Recuperación de imagenes de la query del user 
#

def fetch_images_for_text_docs(
    text_docs: List[Document],
    max_images: int = 8,
) -> List[Document]:
    """
    A partir de los documentos de TEXTO recuperados,
    busca las imágenes que comparten el mismo route_id (u otra key común).
    """
    # 1) Recoger route_id únicos de los docs de texto
    route_ids = []
    ...
    if not route_ids:
        print("⚠️ No se encontró ningún route_id en los documentos de texto.")
        return []

    print(f"🔗 Buscando imágenes para route_id: {route_ids}")

    # 2) Abrir el store de imágenes
    image_store = ChromaDocumentStore(
        persist_path=str(CHROMA_PERSIST_DIR),
        collection_name=IMAGE_COLLECTION,
        distance_function="cosine",
    ...
    # 3) Construir filtros tipo OR sobre meta.route_id
    filters = {
        "operator": "OR",
        "conditions": [
            {
    ...
    # 4) Traer documentos de imágenes que cumplan esos filtros
    image_docs = image_store.filter_documents(filters=filters)

    # Opcional: limitar el nº de imágenes
...
def interactive_multimodal_chat():
    ...
    while True:
        ...
        image_docs_clip = result["image_retriever"]["documents"]
        image_docs_route = fetch_images_for_text_docs(text_docs, max_images=4)

        # Unir, evitando duplicados por id
        seen_ids = set()
        image_docs = []
        for d in image_docs_route + image_docs_clip:
