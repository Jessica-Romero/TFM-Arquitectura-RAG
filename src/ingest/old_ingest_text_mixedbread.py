...
# Modelo de embeddings (Mixedbread)
...
DOCS_SECTIONS_CHUNKS = RAG_DOCS_DIR / "docs_sections_chunks_ve2.csv"
DOCS_WAYPOINTS = RAG_DOCS_DIR / "docs_waypoints_v3.csv"
...
def embed_documents(docs: List[Document]) -> None:
    """
    Calcula embeddings con Mixedbread y los asigna a cada Document.embedding.
    Usa prefijo diferente para query/document, pero aquí todo son documentos.
    """
    if not docs:
        return
    ...
    for doc in docs:
        txt = doc.content or ""
        # Para indexación tratamos todo como "document"
    ...
    # Asignar embeddings a cada Document
    ...
    for _, row in df.iterrows():
        ...
        for col, val in row.items():
            ...
            if col.startswith("meta_"):
                ...
                # Caso especial: tus campos numéricos importantes
...
def main():
    ...
    # 3) Calcular embeddings con Mixedbread
    print("\n Calculando embeddings con Mixedbread...")
    embed_documents(all_docs)  # prueba rápida

    # 4) Configurar DocumentStore (sin embedding_function custom)
    ...
    # 5) Vaciar colección antes de reindexar (API nueva)
    try:
